...
// NEW: comma/space separated list for quick per-guild registration
...
async function registerAll() {
  if (guildIds.length > 0) {
    // Register to multiple guilds (fast propagation)
    for (const gid of guildIds) {
      ...
      await new Promise(r => setTimeout(r, 350)); // polite pacing
    }
    return;
  }
  if (guildId) {
  ...
  // Fallback: global registration (slow to propagate, good for public bots)
